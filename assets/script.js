...
function renderTrackForViewport(track) {
	const originalHtml = track.dataset.originalHtml || '';
	if (!originalHtml) {
		track.innerHTML = '';
		return;
	}

	track.innerHTML = mobileQuery.matches
		? originalHtml
...
fetch(sheetUrl)
	...
	.then(text => {
		...
		track.dataset.originalHtml = cardsHtml;
		renderTrackForViewport(track);
